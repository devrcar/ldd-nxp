...
main() {
    # Choose mode: --cli (default) or --gui
    MODE="cli"
    if [[ "$#" -gt 0 ]]; then
        case "$1" in
            --gui)
            ...
            --cli)
                MODE="cli" ;;
            -h|--help)
                echo "Usage: $(basename "$0") [--cli|--gui]";
                exit 0 ;;
            *)
                echo "Unknown option: $1" >&2;
                echo "Usage: $(basename "$0") [--cli|--gui]" >&2;
    ...
    if [[ "${MODE}" == "cli" ]]; then
        ...
        run_as_root env PYTHONPATH="${USR_DRV_DIR}" python3 -m "${CLI_MODULE}" --test
        USER_RET=$?

        if [ ${USER_RET} -ne 0 ]; then
            echo -e "${BRED}ERROR:${COLOR_OFF} CLI returned non-zero (${USER_RET})"
        else
            echo -e "${BGREEN}INFO:${COLOR_OFF} CLI test completed successfully"
        fi
    else
        # Run the GUI mode
        GUI="${USR_DRV_DIR}/gui/app.py"
        if [ ! -f "${GUI}" ]; then
            echo -e "${BRED}ERROR:${COLOR_OFF} GUI not found at ${GUI}"
            km_cleanup
            exit ${ERR_NOT_A_FILE}
        fi

        GUI_VENV="${USR_DRV_DIR}/.venv"
        if [ ! -d "${GUI_VENV}" ]; then
            echo -e "${BRED}ERROR:${COLOR_OFF} GUI virtual environment not found at ${GUI_VENV}"
            echo -e "${BYELLOW}WARN:${COLOR_OFF} Did you forget to run build.sh?"
            km_cleanup
            exit ${ERR_NOT_A_DIR}
        fi

        GUI_MODULE="gui.app"
        echo -e "${BWHITE}INFO:${COLOR_OFF} Launching GUI (module: ${GUI_MODULE})"

        # Run with Python3 and make sure the user/ directory is on PYTHONPATH so
        # the `backend` package can be imported. Run as root if required so the
        # CLI can open the character device under /dev. Run the Python binary from
        # the GUI virtual environment, so it can find its installed dependencies.
        run_as_root env PYTHONPATH="${USR_DRV_DIR}" "${GUI_VENV}/bin/python" -m "${GUI_MODULE}"
        USER_RET=$?

        if [ ${USER_RET} -ne 0 ]; then
            echo -e "${BRED}ERROR:${COLOR_OFF} GUI returned non-zero (${USER_RET})"
        else
            echo -e "${BGREEN}INFO:${COLOR_OFF} GUI test completed successfully"
        fi
    fi
